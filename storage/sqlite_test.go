...
func TestStorageHyperliquidRoundTrip(t *testing.T) {
	...
	if found {
		t.Fatalf("expected no submission for fresh metadata")
	}
...
func TestStorageListEventsForOrder(t *testing.T) {
	...
	md1, evt1 := mk(0, 123.00, "initial revision")
	md2, evt2 := mk(10*time.Minute, 123.45, "second revision")
	md3, evt3 := mk(25*time.Minute, 124.10, "final revision")
...
func TestOrderScalerUpsertAndGet(t *testing.T) {
	...
	initial, err := store.queries.GetOrderScaler(ctx)
	require.NoError(t, err)
	require.Equal(t, float64(1.0), initial.Multiplier)
	require.Equal(t, "system", initial.UpdatedBy)
	require.Nil(t, initial.Notes)

	note := "manual baseline"
	err = store.queries.UpsertOrderScaler(ctx, sqlcgen.UpsertOrderScalerParams{
		Multiplier: 0.42,
		UpdatedBy:  "tester",
		Notes:      &note,
	})
	...
	updated, err := store.queries.GetOrderScaler(ctx)
	require.NoError(t, err)
	require.Equal(t, float64(0.42), updated.Multiplier)
	require.Equal(t, "tester", updated.UpdatedBy)
	require.NotNil(t, updated.Notes)
	require.Equal(t, note, *updated.Notes)
	require.GreaterOrEqual(t, updated.UpdatedAtUtc, initial.UpdatedAtUtc)
}
...
func TestBotOrderScalerCRUD(t *testing.T) {
	...
	botID := int64(4242)
	err := store.queries.UpsertBot(ctx, sqlcgen.UpsertBotParams{
		BotID:         botID,
		Payload:       []byte("{}"),
		LastSyncedUtc: time.Now().UTC().UnixMilli(),
	})
	require.NoError(t, err)

	multiplier := 0.75
	note := "hedged exposure"
	err = store.queries.UpsertBotOrderScaler(ctx, sqlcgen.UpsertBotOrderScalerParams{
		BotID:      botID,
	...
	override, err := store.queries.GetBotOrderScaler(ctx, botID)
	require.NoError(t, err)
	...
	effectiveFrom := override.EffectiveFromUtc
	firstUpdatedAt := override.UpdatedAtUtc

	overrides, err := store.queries.ListBotOrderScalers(ctx)
	require.NoError(t, err)
	...
	updated, err := store.queries.GetBotOrderScaler(ctx, botID)
	require.NoError(t, err)
	require.Nil(t, updated.Multiplier)
	require.Nil(t, updated.Notes)
	require.Equal(t, "inherit", updated.UpdatedBy)
	require.Equal(t, effectiveFrom, updated.EffectiveFromUtc)
	require.Greater(t, updated.UpdatedAtUtc, firstUpdatedAt)

	err = store.queries.DeleteBotOrderScaler(ctx, botID)
	require.NoError(t, err)

	_, err = store.queries.GetBotOrderScaler(ctx, botID)
...
func TestScaledOrderAuditHistory(t *testing.T) {
	...
	err = store.queries.UpsertDeal(ctx, sqlcgen.UpsertDealParams{
		DealID:       dealID,
		BotID:        botID,
		CreatedAtUtc: now,
		UpdatedAtUtc: now,
	...
	_, err = store.queries.InsertScaledOrder(ctx, sqlcgen.InsertScaledOrderParams{
		Md:                  md,
		DealID:              dealID,
		BotID:               botID,
		OriginalSize:        125.0,
	...
	_, err = store.queries.InsertScaledOrder(ctx, sqlcgen.InsertScaledOrderParams{
		...
		CreatedAtUtc:        now + 3000,
		SubmittedOrderID:    &submittedID,
	})
	require.NoError(t, err)

	byMetadata, err := store.queries.ListScaledOrdersByMetadata(ctx, md)
	require.NoError(t, err)
	require.Len(t, byMetadata, 2)
	require.LessOrEqual(t, byMetadata[0].CreatedAtUtc, byMetadata[1].CreatedAtUtc)
	require.Equal(t, int64(0), byMetadata[0].StackIndex)
	require.Equal(t, int64(1), byMetadata[1].StackIndex)
	require.Nil(t, byMetadata[0].SubmittedOrderID)
	require.Nil(t, byMetadata[1].SubmittedOrderID)

	byDeal, err := store.queries.ListScaledOrdersByDeal(ctx, dealID)
	require.NoError(t, err)
	require.Len(t, byDeal, 3)
	require.Equal(t, "040506", byDeal[2].Md)
