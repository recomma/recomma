...
*Tier-based defaults* (determined by ThreeCommas plan tier):

- `DealWorkers`: Tier-specific concurrent goroutines processing deals (NOT user-configurable)
- `OrderWorkers`: 5 concurrent goroutines for order emission (user-configurable)
- `ResyncInterval`: Tier-specific interval between ProduceActiveDeals cycles (NOT user-configurable)
- `errgroup limit`: Tier-specific concurrent bot processors in ProduceActiveDeals
...
2. Apply tier-specific values for `DealWorkers` and `ResyncInterval`
...
**Note**: `DealWorkers` and `ResyncInterval` are NOT user-configurable to prevent rate limit violations from misconfiguration. These values are determined solely by the plan tier to ensure safe operation within API quota limits.
...
- Automatic tier detection and tier-based value assignment in `main.go`
- Ensure DealWorkers and ResyncInterval are NOT exposed as user-configurable flags
