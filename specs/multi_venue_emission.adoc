...
* *Default wallet alignment.* `storage.DefaultHyperliquidIdentifier` still emits the sentinel wallet, so bootstrap code must either realign the stored wallet or alias the emitter, websocket, and storage layers to that identifier to keep `(venue, wallet)`-keyed lookups working.
