...
Use the order scaler to shrink or expand mirrored exposure without touching the source bot.
Review the xref:operations/order-scaler.adoc[order scaler runbook] for end-to-end guidance on
viewing the global default multiplier, managing per-bot overrides, and understanding the
resulting SSE/audit records.
