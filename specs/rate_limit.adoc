...
The rate limiter operates as a gatekeeper that serializes workflow execution through a reservation mechanism. Only ONE workflow can hold an active reservation at any time.
...
Only one workflow can hold a reservation at any given time. All other workflows wait in a FIFO queue.

*Pessimistic Initial Reservation*::
Workflows reserve conservatively (more slots than likely needed) to ensure they can complete. Once actual requirements are known, they adjust downward.

*Early Release Through AdjustDown*::
When a workflow determines it needs fewer slots than reserved, it immediately releases the excess. This allows queued workflows to start *while the original workflow is still executing*.
...
- `activeReservation`: Single active workflow reservation (can be nil)
  * `workflowID`: Unique identifier for workflow
  * `slotsReserved`: Number of slots claimed
  * `slotsConsumed`: Number of API calls actually made
  * `createdAt`: Timestamp for metrics
- `waitQueue`: FIFO queue of workflow IDs waiting to reserve
...
Behavior:
1. If no active reservation and window has capacity ’ grant immediately
2. If active reservation exists ’ add to wait queue and block
3. When window resets, recalculate capacity before granting
4. FIFO ordering ensures fairness
...
4. Signal waiting workflows that capacity may be available
5. Does NOT release the reservation - workflow continues
...
Behavior:
1. Verify workflowID holds active reservation
2. Calculate if current window has capacity
3. If yes ’ grant immediately, increase `slotsReserved`
4. If no ’ block until next window reset (keep reservation, block queue)
...
Behavior:
1. Remove from activeReservation
2. Signal next workflow in wait queue
3. Next workflow attempts Reserve from step 1
...
2. `consumed` = 0
3. Active reservation is NOT cancelled (workflow continues)
4. Waiting workflows are NOT automatically granted (must re-check capacity)
...
*Key Insight*: AdjustDown at Time 1s allowed HandleDeal(AAA) to start while ProduceActiveDeals was still running. This is the mechanism for overlapping workflow execution without violating the single-reservation principle.
...
*Question*: When checking if next workflow can reserve, is the formula: `(consumed + all_active_reservations + requested) <= limit`?

*Context*: How do we account for reservations vs actual consumption when deciding to grant?

*Impact*: Core correctness of the rate limiter.

=== 4. Extend Across Window Boundary

*Question*: When Extend requires waiting for window reset, should we:
- Keep reservation and wait (blocks queue)
- Release and re-reserve in new window (loses priority)

*Context*: ProduceActiveDeals on Expert with 1000 bots might need multiple windows. Should it hold the reservation across windows?

*Impact*: Fairness vs throughput tradeoff for large workloads.
